...
mydata %>% 
  mutate( m1 =ifelse(mydata$Month==1, 1, 0),
          m2 = ifelse(mydata$Month==2, 1, 0),
          m3 = ifelse(mydata$Month==3, 1, 0),
          m4 = ifelse(mydata$Month==4, 1, 0),
          m5 = ifelse(mydata$Month==5, 1, 0),
          m6 = ifelse(mydata$Month==6, 1, 0),
          m7 = ifelse(mydata$Month==7, 1, 0),
          m8 = ifelse(mydata$Month==8, 1, 0),
          m9 = ifelse(mydata$Month==9, 1, 0),
          m10= ifelse(mydata$Month==10, 1, 0),
          m11= ifelse(mydata$Month==11, 1, 0),
          m12= ifelse(mydata$Month==12, 1, 0)) -> mydata
...
#' 2=haddock,
#' 3=Alaska 
#' such that given pi, then p1 is the price of cod,
#  p2 is the price of haddock etc. 
#' lnp1 is log price for cod. R1 is budget share for cod etc.
...
# check the sum of the shares add up to unity
mydata$R1+mydata$R2+mydata$R3
...
#' (22 in this case. Can be seen in the output above):

qchisq(0.95,22)

#' We're able to reject the null hypothesis in this case since the computed chi square 
#' is larger than the critical chi squared, i.e. the monthly dummies
...
#' with our monthly H0's (think about the substitution matrix):

#' Test Homogeneity 
#' H0: The equations are homogeneous
...
#' Test symmetry: 
#' H0: The equations are symmetric
...
#' To sum up: 
#' we're not able to reject any of the homogeneity and symmetry joint
...
#' can and should discuss a bit (a few lines) why your model and the data does not support 
...
##present estimated coefficients with t value and p value, mean of market share and then elasticities together
#output <- cbind(coeffRelevant, mw, els)
...
#remember, we keep the income elasticity. This does not change. 

#Construct final matrix with expenditure elast.
...
#present estimated coefficients with t value and p value, 
#mean of market share and then elasticities together
# output13 <- cbind(coeffRelevant13, mw13, els13)
